...
public class BlocksBehavior : MonoBehaviour
{
    ...
    void Update()
    {
        if (GetComponentInParent<PlayerInfo>().canSmashNextBlock && n_currentBlocksInGame > 0)
        {
            ...
            {
                foreach (KeyValuePair<string, bool> inputs in GetComponentInParent<PlayerInfo>().allPlayerInputs)
                {
                    if (inputs.Value)
                    {
                        bool isSmashAllowed = false;

                        foreach (GameObject block in GameManager._instance.allInputSmashAllowness[inputs.Key])
                        {
                            if (firstBlock.name == block.name + "(Clone)")
                            {
                                if (firstBlock.name == GameManager._instance.blockSpecial1.name + "(Clone)")
                                {
                                    if (inputs.Key == "Special")
                                    {
                                ...
                                else if (firstBlock.name == GameManager._instance.blockSpecial2.name + "(Clone)")
                                {
                                    if (inputs.Key == "Special")
                                    {
